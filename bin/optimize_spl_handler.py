...
class OneshotOptimizeSPLHandler(PersistentServerConnectionApplication, BaseRestUtils):
    ...
    def handle_oneshot_optimizespl_generation(self, saia_api, params, job_id, locale, should_log_telemetry, source_app_id, system_scoped_service, user):
        """
        Handle the oneshot-optimizespl generation request.
        """
    ...
    def handle_func(self, request):
        ...
        should_log_telemetry, _ = self._fetch_telemetry_details(service, request, "oneshot-optimize-spl")
        ...
        self.logger.info(
            log_kwargs(
                ...
                user=hashed_user,
                source_app=ns["app"],
                chat_id="oneshot-optimize-spl",
                message="Generating one-shot-optimize-spl SPL",
                saia_app_version=app_version,
            )
        )

        saia_api = SaiaApi(service, system_scoped_service, user, "oneshot-optimize-spl", hashed_user)
        return self.handle_oneshot_optimizespl_generation(
            saia_api,
            params,
            job_id,
            locale,
